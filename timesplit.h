void GetTDNucleotideCounts (run_params& p, vector<string>& consensus, const vector<char>& alphabet, vector<int>& var_positions, vector<site2>& ali_stats, vector<string>& seqs);
void SplitSeqs (const vector<int>& times, const vector<int>& times_uniq, const vector<string>& seqs, vector< vector<string> >& seqs_t);
void SplitAliStats (const vector<int>& times_uniq, const vector<char>& alphabet, const vector< vector<string> >& seqs_t, vector< vector<site2> >& ali_stats_t);
